...
(function () {
   ...
   */
  function removeAdChildrenOnly(container, adElementSet, removedSet, removed) {
    const toRemove = [];
    for (const child of container.children) {
      if (adElementSet.has(child) && !removedSet.has(child)) {
        toRemove.push(child);
      }
    }
  ...
  function cleanAdTextFromContainer(container) {
    ...
    while ((textNode = walker.nextNode())) {
      if (!textNode.parentElement || !document.contains(textNode.parentElement))
        continue;
  ...
  function findRemovalTarget(el, adElementSet) {
    ...
    for (let i = 0; i < MAX_LEVELS; i++) {
      ...
      if (isPureContainer(parent, current)) {
        // 纵向收缩成功 → 继续向上
        target = parent;
        current = parent;
      } else {
        // 纵向走不通 → 尝试横向扩展（兄弟可能也是广告项）
  ...
  async function scanAndRemove() {
    ...
    for (const el of adElements) {
      try {
        ...
        if (hasFunctionalContent(target, adElementSet)) {
          // 混合容器：清理广告文字 + 移除广告子元素
  ...
  function interceptExternalRedirects() {
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        e.preventDefault();
        e.stopPropagation();
        console.log(`[广告拦截助手] 已拦截外部跳转: ${href.substring(0, 100)}`);
      },
      true, // 捕获阶段
    );
  }
